...
resource "cloudflare_record" "dns-entry-bootnode" {
  zone_id = data.sops_file.cloudflare.data["zones.ethpandaops-io.zone_id"]
  name    = "withdrawal-devnet-3.${data.sops_file.cloudflare.data["zones.ethpandaops-io.domain"]}"
  type    = "A"
...
resource "cloudflare_record" "dns-entry-bootnode-wildcard" {
  zone_id = data.sops_file.cloudflare.data["zones.ethpandaops-io.zone_id"]
  name    = "*.withdrawal-devnet-3"
  type    = "CNAME"
  value   = "withdrawal-devnet-3.${data.sops_file.cloudflare.data["zones.ethpandaops-io.domain"]}"
  proxied = false
}